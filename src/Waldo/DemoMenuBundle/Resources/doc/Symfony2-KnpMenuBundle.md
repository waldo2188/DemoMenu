...
Ce Bundle permet de créer des menus facilement. La [documentation](https://github.com/KnpLabs/KnpMenuBundle/blob/master/Resources/doc/index.md) est d'ailleurs assez bien faite pour les choses simples. Je vous laisse vous y reporter pour l'installation du bundle dans votre architecture Symfony2.

Ce que je vais vous présenter ici, c'est une gestion un peu plus avancée des menus avec en prime, la possibilité de créer un breadcrumb/fil d'Ariane en deux coups de cuillère à pot.
...
Le but de cette démonstration est de vous présenter la création d'un menu multi-niveaux, avec des entrées cachées. Dans le cas de certaines applications nous ne voulons pas afficher dans notre menu des entrées comme « Ajouter un article » ou « Modifier un article ». Ces entrées seront présentes dans l'application sous forme de bouton dans l'interface. Mais nous souhaitons quand même que ces entrées fassent partie du breadcrumb.
...
Le point important de ce Controller est l'option `defaults` de la *Route*. Cette option permet de donner une valeur par défaut à une variable qui est passée dans l'URL.
...
$menu['Utilisateurs']->addChild('Actif', array('route' => '_utilisateur_actif_list'));
        ...
        $menu['Utilisateurs']->addChild('Éditer', array('route' => '_utilisateur_editer'))
                ->setDisplay(false);
```
Deux sous-branches appellent la méthode `setDisplay`, cela permet de ne pas afficher les entrées dans le menu. Cependant, ces entrées font partie intégrante de l'arborescence, donc très utile pour une génération de Breadcrumbs !
...
Menu version Épicé
------------------
Pour l'instant nous avons créé des entrées de menu simple. Mais que faire dans le cas où une même route affiche différente choses, comme dans le cadre d'un blog. J'ai une route *_article_blog* qui prend en paramètre *idArticle*.
...
Le plus fun, c'est qu'au rendu du menu, l'entrée taggée par la class CSS *current* (c.a.d. Le lien en cours d'utilisation), et bien ce sera la bonne entrée. Le KnpMenuBundle, par le biais de classes ``Voter``, permet grâce à la route et à ces paramètres de faire la correspondance avec ce que vous avez déclaré dans le fichier Builder. Et avec un peu de CSS on peut faire ce que l'on veux !
...
-----------------------
Ce que nous avons vue précédemment n'est qu'un prémice. Corsons les choses en créant la partie *Niveau 0* à *Niveau 5*.
